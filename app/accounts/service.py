...
class AccountsService:
    ...
    def get_by_owner(self, owner: str) -> List[Account]:
        ...
        self.logger.info('get_by_owner: {} returned {} Items'.format(owner, len(resp['Items'])))
    ...
    def get_by_domain(self, domain: str) -> List[Account]:
        ...
        if 'Items' in resp:
            self.logger.info('get_by_domain: {} returned {} Items'.format(domain, len(resp['Items'])))
            for item in resp['Items']:
                account = Account(**item)
                same_domain_accounts.append(account)
        else:
            self.logger.error('get_by_domain: No Items attribute found. domain={}, Unexpected response: {}'
